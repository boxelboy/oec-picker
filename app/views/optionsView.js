...
-------------------------------------------------------------------------------------------------------------------------*/
													...
													switch (this.table) {
														case "simulator":
															saveData['step_key'] = this.stepStuff.models[0].attributes.sequence;
															saveData['simulator_key'] = valueArr[0];
															saveData['option_key'] = this.stuff.models[0].attributes.option_key_primary;
															saveData['step_option'] = this.stuff.models[0].attributes.id;
															itemSaveModel = new simitemsModel(saveData);
															break;
														case "order":
															saveData['option_key'] = this.stuff.models[0].attributes.option_key_primary;
															saveData['step_key'] = this.stepStuff.models[0].attributes.step_key_primary;
															saveData['step_option'] = this.stuff.models[0].attributes.id;
												...
												this.simItem.fetch(
												{
													...
													success: _.bind(function (response){
														//console.log('response after check',response.models[0]);
														...
														{
															if (this.stuff.models[0].get('description_for_tree') === 'Calculation') {
																var parameter = response.models[0].get('id');// + '\r' + theFile;
																$.post('/fm', { action: 'script', file: theFile, script: 'Update_Calculation', parameter: parameter });
																this.cpsLoop(response.models[0].get('id'));
															}
															else {
																this.render(response.models[0]);
															}
...
-------------------------------------------------------------------------------------------------------------------------*/
		cpsLoop: function (data)
		{
			...
			itemGetModel.fetch().done(_.bind(function(getResponse){
				...
				if (typeof getResponse.evaluate !== 'undefined') {
